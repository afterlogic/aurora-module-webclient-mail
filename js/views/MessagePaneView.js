...
const ComposeUtils = require('modules/%ModuleName%/js/utils/Compose.js'),
...
function CMessagePaneView() {
  ...
  App.broadcastEvent(
    '%ModuleName%::AddMoreSectionCommand',
  ...
  App.broadcastEvent(
    '%ModuleName%::RegisterMessagePaneController',
  ...
  this.sQuickReplyViewTemplate =
    App.isMobile() || !Settings.AllowQuickReply ? '' : '%ModuleName%_Message_QuickReplyView'
  ...
  App.broadcastEvent(
    '%ModuleName%::AddAllAttachmentsDownloadMethod',
  ...
  this.placeholderText = ko.computed(function () {
    return this.hasReplyAllCcAddrs()
      ? TextUtils.i18n('%MODULENAME%/LABEL_QUICK_REPLY_ALL')
      : TextUtils.i18n('%MODULENAME%/LABEL_QUICK_REPLY')
  }, this)
  this.sendButtonText = ko.computed(function () {
    return this.hasReplyAllCcAddrs()
      ? TextUtils.i18n('%MODULENAME%/ACTION_SEND_ALL')
      : TextUtils.i18n('%MODULENAME%/ACTION_SEND')
  }, this)
  ...
  this.saveButtonText = ko.computed(function () {
    return this.replyAutoSavingStarted()
      ? TextUtils.i18n('%MODULENAME%/ACTION_SAVE_IN_PROGRESS')
...
CMessagePaneView.prototype.ViewTemplate = App.isNewTab()
  ? '%ModuleName%_MessagePaneScreenView'
  : '%ModuleName%_MessagePaneView'
  ...
  $($('blockquote', $(this.domTextBody())).get()).each(function () {
    var $blockquote = $(this),
      ...
      $switchButton = $('<span class="blockquote_toggle"></span>').html(
        TextUtils.i18n('%MODULENAME%/ACTION_SHOW_QUOTED_TEXT')
      ),
      bHidden = true
