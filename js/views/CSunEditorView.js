...
CHtmlEditorView.prototype.ViewTemplate = '%ModuleName%_SunEditorView';
...
CHtmlEditorView.prototype.init = function (sText, bPlain, sTabIndex, sPlaceholderText)
{
	...
	if (this.oSunEditor && $('#' + this.editorId).children().length === 0)
	{
		this.oSunEditor.destroy();
		this.oSunEditor = null;
		// in case if knockoutjs destroyed dom element with html editor
		// if ()
		// {
			// this.oSunEditor.start(this.isEnable());
	...
	if (!this.oSunEditor)
	{
		...
		this.oSunEditor = suneditor.create(this.editorId, {
			plugins: plugins,
	...
	window.suneditor = this.oSunEditor;
...
{
	// TODO
	if (this.oSunEditor && !this.disableEdit())
	{
		const 
			content = this.oSunEditor.getContext().element.wysiwyg,
...
CHtmlEditorView.prototype.getPlainText = function ()
{
	...
	if (this.oSunEditor)
	{
		return this.oSunEditor.getText();
	}
...
CHtmlEditorView.prototype.getText = function (bRemoveSignatureAnchor)
{
	...
	var
		...
		// sText = this.oSunEditor ? this.oSunEditor.getContents(bRemoveSignatureAnchor) : ''
		sText = this.oSunEditor ? this.oSunEditor.getContents(true) : ''
	;
...
{
	return $(this.oSunEditor.getContext().element.wysiwyg);
};
...
CHtmlEditorView.prototype.setText = function (sText, bPlain)
{
	...
	if (this.oSunEditor && !this.disableEdit())
	{
		if (bPlain) {
			...
			// TODO fix formatting
			this.oSunEditor.setContents('<p>'+sText+'</p>');
		} else {
			this.oSunEditor.setContents(sText);
		}
...
CHtmlEditorView.prototype.undoAndClearRedo = function ()
{
	...
	if (this.oSunEditor)
	{
		const history = this.oSunEditor.core.history;
		history.undo();
		history.stack.splice(history.getCurrentIndex(), history.stack.length);
		//history.reset(true);
		// this.oSunEditor.undo();
		// this.oSunEditor.clearRedo();
	}
};
...
CHtmlEditorView.prototype.clearUndoRedo = function ()
{
	...
	if (this.oSunEditor)
	{
		this.oSunEditor.core.history.reset();
	}
};
...
{	
	...
	return this.oSunEditor ? true : false;
};
...
CHtmlEditorView.prototype.insertHtml = function (sHtml)
{
	...
	if (this.oSunEditor)
	{
		if (!this.oSunEditor.core.hasFocus)
		{
			this.oSunEditor.core.focus();
		}
		...
		this.oSunEditor.insertHTML(sHtml);
	}
};
